...
class HAAML(nn.Module):
    ...
    def forward(self, x, label):
        ...
        cos_theta_m = gt * self.cos_m - sin_theta * self.sin_m #(old)
        ...
        with torch.no_grad():
            new_m = self.m_0 + self.t * torch.log(H + 1) # v3
            cos_new_m = torch.cos(new_m)
            sin_new_m = torch.sin(new_m)

        # cos(theta + new_m)
        cos_theta_newm = gt * cos_new_m - sin_theta * sin_new_m
        new_gt = torch.where(gt > 0, cos_theta_newm , gt)
        cos_theta.scatter_(1, label.view(-1, 1), new_gt)
        output = cos_theta * self.s

        # regularizer
        hard_regularizer = self.loss_weight * H

        return output, hard_regularizer, gt
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loss_function', type=str, default='HAAML_Laplace')
    ...
    parser.add_argument('--t', type=float, default=0.5, help='base margin')
    parser.add_argument('--loss_weight', type=float, default=15, help='base margin')
    ...
    for seed in args.seed_list:
        ...
        exp_lr_scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[9, 18, 22], gamma=0.1)
        ...
        for epoch in range(start_epoch + 1, args.total_epochs + 1):
            ...
            for data in trainloader:
                ...
                optimizer.zero_grad()
                raw_logits = net(img)
                output, hard_regular, score = loss_function(raw_logits, label)  # cos_theta, gt
                score = score[score != -2]
                clean_output = output[score > thresh_value_sim]
                clean_hard_regular = hard_regular[score > thresh_value_sim]
                ...
                total_loss = criterion(clean_output, label_) + clean_hard_regular.mean()
                total_loss.backward()
                optimizer.step()
                ...
                if total_iters % 500 == 0:
                    ...
                    since = time.time()
                    _print(
                        "Iters: {:0>6d}/[{:0>2d}], loss: {:.4f}, hard_loss: {:.4f},train_accuracy: {:.4f}, time: {:.2f} s/iter, learning rate: {}".format(
                            total_iters, epoch, total_loss.item(), clean_hard_regular.mean(),
            ...
            with torch.no_grad():
                for batch_idx, (image, label, lmdb_keys) in enumerate(trainloader):
                    ...
                    _, _h, score = loss_function(feat, label)
                ...
                _print('num of outliner:%d' % (len_outliner))
